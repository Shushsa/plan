...
import (
	...
	structure "github.com/plan-crypto/node/x/structure/keeper"
...
var (
	...
	initCoins     = sdk.NewCoins(sdk.NewCoin(plnTypes.PLN, initTokens))
)
...
func TestIncreaseStructureBalance(t *testing.T) {
	...
	// The latest account in the structure gets 0.001 PLN
	coinsAmount = sdk.NewInt(1000)
...
func TestDecreaseStructureBalance(t *testing.T) {
	...
	// The latest account gets 0.001 PLN
	coinsAmount = sdk.NewInt(1000)
	keeper.IncreaseStructureBalance(ctx, app.addrs[0], coinsAmount)

	// The latest account transfered 0.0004 PLN to another structure, now it has just 0.0006 PLN in the structure
